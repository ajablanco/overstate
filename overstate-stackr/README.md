...
```bash
# For Linux
docker build -t overstate-counter:latest .

# For Mac with Apple Silicon chips
docker buildx build --platform linux/amd64,linux/arm64 -t overstate-counter:latest .
```
...
docker run --env-file .env -v ./db.sqlite:/app/db.sqlite -p <HOST_PORT>:<CONTAINER_PORT> --name=overstate-counter -it overstate-counter:latest

# If using other URI based datastores
docker run --env-file .env -p <HOST_PORT>:<CONTAINER_PORT> --name=overstate-counter -it overstate-counter:latest
```
